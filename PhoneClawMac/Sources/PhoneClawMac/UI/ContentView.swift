...
struct ContentView: View {
    ...
    /// 记录每个 skill 卡片的展开状态（key = ResponseBlock.id）
    ...
    private var topBar: some View {
        HStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                Text(engine.llm.isLoaded ? "Gemma 4 E4B" : engine.llm.statusMessage)
    ...
    private func send() async {
        let text = inputText
        inputText = ""
        await engine.processInput(text)
    }
}
