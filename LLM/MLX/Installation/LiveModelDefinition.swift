...
//
// ASR (sherpa-onnx-streaming-zipformer-zh) + TTS (vits-zh-hf-keqing) 的元信息。
// 用于手机端按需下载 LIVE 语音模型，与 LLM 下载基础设施并列但独立。
// 注: VAD (Silero) 由 FluidAudio 自行管理，不在此定义。
...
enum LiveModelDefinition {
    ...
    static let all: [LiveModelAsset] = [asr, tts]

    /// 合并 ID，用于 UI 展示和状态管理
    static let combinedID = "live-voice-models"

    /// 总大小估算 (用于 UI 提示)
    static let estimatedSizeMB = 300  // ASR ~167MB + TTS ~136MB
