...
enum LiveDownloadPlanner {
    ...
    static func makePlan(for asset: LiveModelAsset) async throws -> LiveAssetDownloadPlan {
        ...
        for host in hosts {
            do {
                let repositoryFiles = try await fetchTreeRecursive(host: host, repo: asset.repositoryID, path: "")
    ...
    private static func fetchTreeRecursive(host: String, repo: String, path: String) async throws -> [RepositoryFile] {
        ...
        for item in items {
            if item.type == "file" {
                ...
                let subFiles = try await fetchTreeRecursive(host: host, repo: repo, path: item.path)
    ...
    private static func downloadSources(for asset: LiveModelAsset, file: String) -> [DownloadFile.Source] {
