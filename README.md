...
注：现在这个版本空壳IPA还有问题，我不是付费开发者，IPA包内存受限在3GB以内，运行E2B最低也要3GB以上，所以会闪崩，付费开发者申请还要很久，请大家还是尽量用原生XCode打包。

## 2026-04-08 更新

- 模型下载新增 ModelScope 国内镜像，国内用户无需 VPN 即可下载 Gemma 4
- 大幅重构内存管理：推理预算改为按实际可用内存动态计算，长 prompt 和长回答不再被错误截断
- 单轮工具调用相对稳定，多轮工具调用还在继续优化
